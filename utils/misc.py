...
class DoubleMetricLogger(object):
    def __init__(self, delimiter='  '):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.iter_end_t = time.time()
    ...
    def log_every(self, start_it, max_iters, itrt, new_itrt, print_freq, header=None):
        # itrt: data loader
        self.log_iters = set(np.linspace(0, max_iters-1, print_freq, dtype=int).tolist())
        self.log_iters.add(start_it)
        if not header:
        ...
        if isinstance(new_itrt, Iterator) and not hasattr(new_itrt, 'preload') and not hasattr(new_itrt, 'set_epoch'):
            for i in range(start_it, max_iters):
                obj = next(itrt)
                new_obj = next(new_itrt)
                self.data_time.update(time.time() - self.iter_end_t)
                yield i, obj, new_obj
                self.iter_time.update(time.time() - self.iter_end_t)
                if i in self.log_iters:
            ...
            if isinstance(new_itrt, int): new_itrt = range(new_itrt)
            for i, obj in enumerate(new_itrt):
                self.data_time.update(time.time() - self.iter_end_t)
                yield i, obj
                self.iter_time.update(time.time() - self.iter_end_t)
                if i in self.log_iters:
