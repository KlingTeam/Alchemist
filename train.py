...
# from dataset.data import build_dataset,build_dataset_webtar,gather_file_keys,build_dataset_csv
from dataset.data import build_dataset_csv
from models.text_encoder import build_text
from utils.data_sampler import DistInfiniteBatchSampler, EvalDistributedSampler, DistInfiniteWeightedBatchSampler
...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        if args.using_webtar:
            ...
            ld_val = DataLoader(
                ...
                shuffle=False, drop_last=False, worker_init_fn=init_worker
            )
            
            del dataset_val
            if hasattr(args, 'using_dynamic_weight') and args.using_dynamic_weight:
                dataset_len = len(dataset_train)
                num_samples = (args.sample_portion * dataset_len) // 100
                initWeights = get_dynamic_weights(dataset_len, 0, args.ep)
                print('initWeight shape: ', initWeights.shape)
                ld_train = DataLoader(
                    dataset=dataset_train, num_workers=args.workers, pin_memory=True,
                    generator=args.get_different_generator_for_each_rank(), # worker_init_fn=worker_init_fn,
                    batch_sampler=DistInfiniteWeightedBatchSampler(
                        dataset_len=len(dataset_train), num_samples=num_samples, weights=initWeights,
                        glb_batch_size=args.glb_batch_size, same_seed_for_all_ranks=args.same_seed_for_all_ranks,
                        fill_last=True, rank=dist.get_rank(), world_size=dist.get_world_size(), start_ep=start_ep, start_it=start_it,
                    ),
    ...
    return (
        ...
        iters_train, ld_train_iter, ld_train, ld_val, text_encoder
    )
...
def main_training():
    ...
    (
        tb_lg, trainer,
        start_ep, start_it,
        iters_train, ld_train_iter, ld_train, ld_val,
        text_encoder
    ...
    for ep in range(start_ep, args.ep):
        if hasattr(args, 'using_dynamic_weight') and args.using_dynamic_weight and ep > 0:
            new_weights = get_dynamic_weights(ld_train.batch_sampler.dataset_len, ep, args.ep)
            ld_train.batch_sampler.reset_weights(new_weights)
            print('new_weights shape: ', new_weights.shape)
            ld_train_iter = iter(ld_train)
        if hasattr(ld_train_iter, 'sampler') and hasattr(ld_train_iter.sampler, 'set_epoch'):
            print(f"has attr sampler")
            ld_train_iter.sampler.set_epoch(ep)
            if ep < 3:
                print(f"ep < 3")
                # noinspection PyArgumentList
                print(f'[{type(ld_train_iter).__name__}] [ld_train_iter.sampler.set_epoch({ep})]', flush=True, force=True)
        ...
        stats, (sec, remain_time, finish_time) = train_one_ep(
            ...
            args=args, tb_lg=tb_lg, ld_or_itrt=ld_train_iter, text_enc=text_encoder, iters_train=iters_train, trainer=trainer
        )
        print(f'===========> epoch:{ep}, after eval')
        ...
        if (ep + 1) % 10 == 0 or (ep + 1) == args.ep:#每10 epoch验证一次
    ...
    del stats
    del iters_train, ld_train, ld_train_iter
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, text_enc, iters_train: int, trainer):
    ...
    for it, obj in me.log_every(start_it=start_it, 
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            precomputed_latent=args.precomputed_latent
        )
