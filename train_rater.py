...
from models import VAR, VQVAE, build_vae_var,build_varRater, VARRater
from trainer import VARTrainer, VARRaterTrainer
...
def build_everything(args: arg_util.Args):
    ...
    if hasattr(args, 'warmupVAR'):
        ckpt = torch.load(args.warmupVAR, map_location='cpu')
        new_state_dict=apply_lvl_emb_and_pos_1LC(args, state_dict=ckpt['trainer']["var_wo_ddp"], patch_nums=args.patch_nums)
        missing, unexpected=var_wo_ddp.load_state_dict(new_state_dict, strict=False)
        print(f'[INIT] Load warmup var model from {args.warmupVAR}\n\n')
    if trainer_state is not None and len(trainer_state):
        ...
        missing, unexpected=var_wo_ddp.load_state_dict(new_state_dict, strict=False)
        missing_, unexpected_ = None, None
        if "varRater_wo_ddp" in trainer_state.keys():
            new_state_dict_rater=apply_lvl_emb_and_pos_1LC(args,state_dict=trainer_state["varRater_wo_ddp"],patch_nums=args.patch_nums)
            missing_, unexpected_=varRater_wo_ddp.load_state_dict(new_state_dict_rater, strict=False)
        print('checkpoints incompatible: ',missing,unexpected, missing_, unexpected_)

    if not args.from_scratch:
        zero_out_cross_attention_weights(var_wo_ddp)
        zero_out_cross_attention_weights(varRater_wo_ddp)
    ...
    opt_kw_rater = dict(lr=args.tlr, weight_decay=0)
    print(f'[INIT] optim={opt_clz_rater}, opt_kw={opt_kw_rater}\n')
  
    varRater_optim = AmpOptimizer(
        mixed_precision=args.fp16, optimizer=opt_clz_rater(params=para_groups_rater, **opt_kw_rater), names=names_rater, paras=paras_rater,
        grad_clip=args.tclip, n_gradient_accumulation=args.ac
    )
    del names_rater, paras_rater, para_groups_rater


    # build trainer
    trainer_rater = VARRaterTrainer(
        device=args.device, patch_nums=args.patch_nums, resos=args.resos,
        vae_local=vae_local, var_wo_ddp=var_wo_ddp, var=var, var_opt=var_optim,
        varRater_wo_ddp=varRater_wo_ddp, varRater=varRater,
        varRater_opt=varRater_optim, label_smooth=args.ls,
        ul_weight=args.upperlevel_weight, ul_weight_decay=args.upperlevel_weight_decay,
    )
    del vae_local, var_wo_ddp, var, var_optim
    del varRater_wo_ddp, varRater, varRater_optim
    ...
    for name, param in trainer_rater.vae_local.named_parameters():
        print(f"Parameter: {name}, Requires Grad: {param.requires_grad}")
    for name, param in trainer_rater.var_wo_ddp.named_parameters():
        print(f"Parameter: {name}, Requires Grad: {param.requires_grad}")
    for name, param in trainer_rater.varRater_wo_ddp.named_parameters():
        print(f"Parameter: {name}, Requires Grad: {param.requires_grad}")

    dist.barrier()
    return (
        tb_lg, trainer_rater, start_ep, start_it,
        iters_train, ld_train, iters_val, ld_val, text_encoder
    )
...
def main_training():
    ...
    (
        tb_lg, trainer,
        start_ep, start_it,
        iters_train, ld_train, iters_val, ld_val,
        text_encoder
    ) = build_everything(args)
    if hasattr(args, "rater_reg"):
        rater_reg = args.rater_reg
    else:
        rater_reg = False
    if hasattr(args, "fixVar"):
        fixVar = args.fixVar
    else:
        fixVar = False
    # train
    ...
    print("in line 313 start_ep ", start_ep)
    for ep in range(start_ep, args.ep):
        if ep>=1:
            trainer.ul_weight -= trainer.ul_weight_decay
            print("\n UL weight now", trainer.ul_weight, end="\n")
        ...
        if hasattr(ld_val, 'sampler') and hasattr(ld_val.sampler, 'set_epoch'):
            ld_val.sampler.set_epoch(ep)
            if ep < 3:
                print(f"ep < 3")
                # noinspection PyArgumentList
                print(f'[{type(ld_val).__name__}] [ld_val.sampler.set_epoch({ep})]', flush=True, force=True)
        print(f"===========> in main training")
        tb_lg.set_step(ep * iters_val)

        stats, (sec, remain_time, finish_time) = train_one_ep(
            ep=ep, is_first_ep=(ep == start_ep), start_it=start_it if ep == start_ep else 0, 
            args=args, tb_lg=tb_lg, ld_or_itrt=ld_train, text_enc=text_encoder, iters_train=iters_train, trainer=trainer, new_ld_or_itrt=ld_val, new_iters=iters_val, rater_reg=rater_reg, fixVar=fixVar
        )
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, text_enc, iters_train: int, trainer, new_ld_or_itrt, new_iters, rater_reg, fixVar):
    # import heavy packages after Dataloader object creation
    # ld_or_itrt:dataloader
    print(f'===========> epoch:{ep}, train_one_ep')
    from trainer import VARRaterTrainer
    from utils.lr_control import lr_wd_annealing
    trainer: VARRaterTrainer
    
    step_cnt = 0
    me = misc.DoubleMetricLogger(delimiter='  ')
    ...
    g_it, max_it = ep * new_iters, args.ep * new_iters
    

    for it, obj, new_obj in me.log_every(start_it=start_it, 
                                         max_iters=new_iters, 
                                         itrt=ld_or_itrt,
                                         new_itrt=new_ld_or_itrt,
                                         print_freq=math.floor(new_iters/args.print_every),
                                         header=header):
        ...
        if args.precomputed_latent:
            ...
            for embed_idx in range(len(prompt_embeds)):
                prompt_embeds[embed_idx] = prompt_embeds[embed_idx].to(args.device, non_blocking=True)
        else:
        ...
        new_inp = new_obj['image'].to(args.device, non_blocking=True)
        if args.precomputed_latent:
            new_prompt_embeds=new_obj['prompt_embeds']
            for embed_idx in range(len(new_prompt_embeds)):
                new_prompt_embeds[embed_idx] = new_prompt_embeds[embed_idx].to(args.device, non_blocking=True)
        else:
            new_obj['prompt_embeds']=text_enc.extract_text_features(new_obj['prompt'])
            new_prompt_embeds=new_obj['prompt_embeds']
        new_B=new_inp.shape[0]
        new_label = torch.tensor([args.default_label]*new_B).to(args.device, non_blocking=True)
        
        args.cur_it = f'{it+1}/{new_iters}'
        ...
        # NOTE: train rater using val data, thus inp_B3HW=new_inp(val data), new_inp_B3HW=inp(inner model train data)
        grad_norm, scale_log2 = trainer.train_step(
            it=it, g_it=g_it, stepping=stepping, metric_lg=me, tb_lg=tb_lg,
            inp_B3HW=new_inp, label_B=new_label, prompt_embeds=new_prompt_embeds,
            prog_si=prog_si, prog_wp_it=args.pgwp * new_iters, new_inp_B3HW=inp, new_label_B=label, new_prompt_embeds=prompt_embeds,
            precomputed_latent=args.precomputed_latent, rater_reg=rater_reg, fixVar=fixVar
        )
    ...
    try:
        if dist.is_local_master():
            local_out_ckpt = os.path.join(args.local_out_dir_path, 'ar-ckpt-ep%d.pth'%(ep))
            print(f'[saving ckpt] ...', end='', flush=True)
            torch.save({
                'epoch':    ep+1,
                'iter':     0,
        ...
        print(f'[saving ckpt] ...', end='', flush=True)
        print(f'     [saving ckpt] failed!  @ {local_out_ckpt}', flush=True, clean=True)
        print(e)
