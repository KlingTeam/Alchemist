...
class VAR(nn.Module):
    ...
    def _print_parameter_stats(self):
        ...
        print(f'  Non-trainable:        {total_params - trainable_params:,} ({(total_params - trainable_params) / 1e6:.2f}M)')
...
class BatchHead(nn.Module):
    def __init__(self, C, hidden_dim=64, act=nn.GELU()):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(C*2, hidden_dim),
            act,
...
class VARRater(nn.Module):
    def __init__(
        ...
        # batch-level head
        self.head_batch = BatchHead(self.C)
        ##TODO#####################
